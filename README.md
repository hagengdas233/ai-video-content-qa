...
2. 普通上传会在服务端计算最终文件 MD5，并将视频上传到 MinIO；不信任前端摘要。
3. 分片上传流程中，后端创建 `uploadId`，Redis 记录文件名、分片总数和用户信息。
4. 每个分片上传成功后，Redis Set 记录已上传分片下标。
5. 合并上传时，后端校验分片数量，在写出最终合并文件的同一条流中同步计算 MD5，再上传到 MinIO，不会为了摘要重复读取大文件。
6. MySQL `media_files.content_hash` 保存最终内容指纹事实，Redis `media:md5:{mediaId}` 保存 24 小时读取缓存。

### 异步视频分析

1. 客户端调用正式分析接口 `POST /media/analyze/{mediaId}`。
2. `JwtInterceptor` 验证 Bearer JWT 的签名和有效期，确认 Token 用户仍存在，并将 `userId` 写入 `UserContext`。
3. 按 Redis -> MySQL -> 历史对象重新计算的顺序解析 `contentHash`；MySQL 命中后回写 Redis，历史回填由 `lock:content-hash:{mediaId}` 避免并发重复计算，并且先写 MySQL 再写缓存；数据库回填失败时清除 Redis-only 摘要。
4. 使用 Redisson RateLimiter 对 AI 分析入口做全局限流。
5. 使用 Redis `analysis:active:{userId}:{contentHash}` 防止同一用户重复提交，同时避免不同用户的相同视频互相阻塞。
6. 将包含 `userId` 和 `contentHash` 的 `AnalysisTaskMsg` 投递到 RocketMQ 的 `video-analysis-topic`。
7. RocketMQ 消费者收到任务后，使用 `lock:analysis:{userId}:{contentHash}` Redisson 分布式锁兜底防重复消费。
...
已有数据库不要重新建库，升级时只执行一次：

```sql
source docs/sql/migrate_add_media_content_hash.sql;
```

新增的 `media_files.content_hash` 使用 `CHAR(32) NULL`，保存服务端生成的小写 MD5。字段允许 `NULL` 以兼容历史记录，不设置唯一约束。当前分析链路先按 `mediaId` 查询媒体记录，再读取实体中的 `content_hash`，没有 `(user_id, content_hash)` 查询，因此本次没有添加无实际用途的组合索引。
...
只需填写测试用户名、密码和测试媒体 ID 占位符。JWT 由 HTTP Client 在登录请求后保存为运行时变量，不要把真实 Token、密码或 JWT 密钥写入仓库。过期测试可临时用 `JWT_EXPIRATION=1` 启动后端，验证后必须恢复正常配置。首次提交和重复提交应连续执行；可结合 RocketMQ 消费日志以及 Redis 的 `analysis:active:{userId}:{contentHash}` 检查只投递一次。`test.http` 还给出了 Redis 回源 MySQL、历史回填、同用户重复提交和不同用户相同内容的检查步骤。
